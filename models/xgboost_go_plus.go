...
func LoadXGBoostByRedis(
	modelKey string,
	mapKey string,
	numClasses int,
	activation activation.Activation,
	red *redis.Pool) (*inference.Ensemble, map[string]int, error) {
	...
	modelBytes, err := redis.Bytes(red.Get().Do("GET", modelKey))
	if err != nil {
		return nil, nil, err
	}
	err = json.Unmarshal(modelBytes, &xgbEnsembleJSON)
	if err != nil {
		return nil, nil, err
	}
	mapBytes, _ := redis.Bytes(red.Get().Do("GET", mapKey))
	err = json.Unmarshal(mapBytes, &featMap)
	if err != nil {
		return nil, nil, err
	}

	if val, ok := featMap["_maxDepth_"]; ok {
		maxDepth = val
	}
	if maxDepth < 0 {
		return nil, nil, fmt.Errorf("max depth cannot be smaller than 0: %d", maxDepth)
	}
	nTrees := len(xgbEnsembleJSON)
	if numClasses <= 0 {
		return nil, nil, fmt.Errorf("num class cannot be 0 or smaller: %d", numClasses)
	}
	if nTrees == 0 {
		return nil, nil, fmt.Errorf("no trees in file")
	} else if nTrees%numClasses != 0 {
		return nil, nil, fmt.Errorf("wrong number of trees %d for number of class %d", nTrees, numClasses)
	}
	...
	for i := 0; i < nTrees; i++ {
		...
		if err != nil {
			return nil, nil, fmt.Errorf("error while reading %d tree: %s", i, err.Error())
		}
	...
	return &inference.Ensemble{EnsembleBase: e, Activation: activation}, featMap, nil
}

func LoadXGBoostFromLocalJSON(
	modelPath,
...
func buildTree(xgbTreeJSON *xgboostJSON, maxDepth int, featureMap map[string]int) (*xgbTree, int, error) {
	...
	for len(stack) > 0 {
		...
		if stackData.Children == nil {
			...
			featIdx, err := convertFeatToIdx(featureMap, featCnt, stackData.SplitFeatureID)
			if err != nil {
				return nil, 0, err
			}
...
func convertFeatToIdx(featureMap map[string]int, featIdx int, feature string) (int, error) {
	if featureMap != nil {
